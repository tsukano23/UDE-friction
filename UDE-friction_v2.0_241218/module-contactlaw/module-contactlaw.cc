	...
	const VectorHandler& XPrimeCurr)
{
	...
	*/
	...
	//ベクトル
	////法線ベクトル
	pexv.normal_vec(normal_vec);
	////internode(一つの質点では考えなくてよい)
	////lateralnode(一つの質点では考えなくてよい)
	////axialnode(一つの質点の場合、質点の速度方向でよい)
	Vec3 axial_vec 		= v1;
	Vec3 axial_unitvec	= v1/v1.Norm();
	Vec3 friction_vec 	= -axial_unitvec;

	//step関数で摩擦力の推移表現
	doublereal dcrit 	= 2.5; 
	doublereal x_tanh   = v1.Norm()/vt;
	doublereal friction_abs	= ptanhf.tanh(x_tanh, dcrit)*nu*F;
	Vec3 f_friction 	= Vec3(
							friction_abs*friction_vec.dGet(1),
							friction_abs*friction_vec.dGet(2),
							friction_abs*friction_vec.dGet(3) + F
	);
